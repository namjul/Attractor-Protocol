...
This file encodes how to **understand**, **reason about**, and **intervene in** this system through disciplined epistemic practice. It is a generative guide for making changes that increase coherence, adaptability, and life in the system while maintaining contact with reality.

Agents should treat this file as orientation before proposing or applying any non-trivial change.
...
- **Human = Pilot** - Sets aims, constraints, values, and final judgment about what constitutes "more alive."
...
This system is a living, unfolding drawing environment. It is a place where meaning, structure, and form emerge through interaction. Drawing is treated as a way of thinking and sensing relationships, not merely as mark-making.
...
- Gradual emergence of form rather than upfront specification
...
**Principle:** Understanding precedes action. Agents reveal assumptions before executing, allowing misunderstandings to be caught early.

**Why this matters:** Agents do not reliably understand instructions on the first attempt. Making understanding explicit prevents wasted work from acting on wrong assumptions.

**Pattern:** For non-trivial tasks, agents respond with Understanding, TL;DR, and Plan before changing files.

### The Planning Response Pattern

For any non-trivial task (code changes, refactors, architectural decisions, schema changes, multi-file edits), agents **first respond** with:

**1. Understanding**
Concise restatement of what the agent believes the user asked for, in its own words.
...
**2. TL;DR**
Short summary (2–5 bullets or sentences) of the proposed approach and expected outcome.

**3. Plan**
Step-by-step plan describing how the task would be addressed, including:
...
**After planning, the agent waits for the human to confirm or correct before changing files.**

### Valid Exceptions (When to Skip Planning)

Skip planning for:

1. **Trivial mechanical changes** - Typo fixes, formatting, simple variable renames with clear scope
2. **Explicitly scoped one-line changes** - User specifies exact file and exact change
3. **When human explicitly requests immediate action** - "Just do X" or "Skip the plan"
4. **Simple lookups or information requests** - No changes being made

### Contact Test for "Non-Trivial"

**A task is non-trivial if it involves:**
- Touching more than one file
- Making more than one conceptual decision
- Affecting any identified center
- Changing system behavior (not just formatting/style)

**When ambiguous:** Default to planning. The cost of unnecessary planning (a few seconds) is lower than the cost of acting on misunderstanding (minutes to hours of wasted work).
...
1. **State what changed** - "New evidence: [usage data / user feedback / implementation result]"
2. **State what no longer holds** - "Previous assumption [Y] is now undermined because..."
3. **State the revised belief** - "Updated understanding: [Z]"
4. **Explain the update** - "This changes our model of [centers/fields/gestures] because..."
...
- Tooling configuration: `rsbuild.config.ts`, `biome.json`, `tsconfig.json`

### Build, Test, and Development Commands

- `npm run dev`: Rsbuild dev server with HMR and Tailwind CLI in watch mode
...
- Pull requests should include:
  - Concise summary using EAI loop structure (Aim, Claim, Contact Test)
...
- **Premature abstraction** - Contact test: Can you point to three concrete cases this abstraction serves?
- **Configuration before behavior stabilizes** - Contact test: Has this behavior been observed in at least three real usage sessions?
- **Optimizing for scale before patterns of use are visible** - Contact test: Do you have usage data indicating current implementation is a bottleneck?
- **Treating patterns as rules rather than observations** - Contact test: Can you name a valid exception to this pattern?
- **Reducing exploratory capacity in the name of precision** - Contact test: Did users discover unplanned uses of the previous version?
- **Mythology about centers, life, or wholeness** - Contact test: Can you specify observables that would falsify this claim?
...
See [GLOSSARY.md](./attractor-protocol.GLOSSARY.md) for definitions of: Pattern, Principle, Rule, Authority, Protocol Evolution.

### Disagreement Resolution Process

When patterns conflict (Person A observes X, Person B observes Y):

1. **Specify observations:** Both parties state what they observed and why
2. **Identify distinguishing observable:** What would we see if X applies vs if Z applies?
3. **Design contact test:** Smallest experiment to distinguish
4. **Evaluate evidence:** Which pattern actually fits? Do we need to split contexts?
5. **Update protocol** with learning

---

### Power Dynamics: Essential Constraints

**Core claim:** When patterns become rules, conflicts get resolved by power rather than evidence, and the discouraged party stops contributing observations.

**Valid use of seniority:**
- "I've observed this pattern fail in the past because [specific evidence]. Here's what happened: [outcome]."
- "Let me share context you might not have: [technical constraint / user feedback]. Does that change how we evaluate?"
...
- "You're too junior to understand why this matters."

**Success indicators (healthy culture):**
...
- Exceptions to patterns celebrated as learning opportunities, not violations
- Disagreements take >5 minutes (time for evidence gathering, not instant authority)

**Failure indicators (discouragement anti-pattern):**
...
- No protocol updates based on junior observations in 6+ months

**If you notice discouragement happening:**
1. Name it explicitly
2. Check for evidence vs. authority
3. Invert the dynamic: "What would convince you that Person B's pattern is correct? Let's design that test."
4. Update the protocol based on the conflict
5. Celebrate revision

**Meta-warning:** If Attractor Protocol is used to end debates via authority, discourage observations from less powerful people, or prevent evolution based on new evidence, **the protocol has become mythology itself.** Return to foundation principles or stop using it entirely.
